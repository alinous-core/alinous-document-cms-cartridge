...
var nicEditor = bkClass.extend({
	...
	getIcon : function(iconName,options) {
		...
		return {'background-repeat' : 'no-repeat', backgroundImage : "url('"+((icon) ? this.options.iconsPath : file)+"')", backgroundPosition : ((icon) ? ((icon-1)*-18) : 0)+'px 0px'};	
	},
...
var nicEditorInstance = bkClass.extend({
	...
	construct : function(e,options,nicEditor) {
		...
		if(isTextarea || this.options.hasPanel) {
			...
			editorHtmlEncoder.convertEditorAvailable(this.elm);
			editorAlinousEncoder.convertEditorAvailable(this.elm);
		}
	...
	getContent : function() {
		editorHtmlEncoder.decode(this.getElm());
		editorAlinousEncoder.decode(this.getElm());
		
		this.content = this.getElm().innerHTML;
		
		editorHtmlEncoder.convertEditorAvailable(this.getElm());
		editorAlinousEncoder.convertEditorAvailable(this.getElm());
	...
	addForm : function(f,elm, submit) {
		...
		for(itm in f) {
			...
			if(type == 'title') {
				...
				if(field.txt) {
					new bkElement('label').setAttributes({'for' : itm}).setContent(field.txt).setStyle({margin : '2px 4px', fontSize : '13px', width : (this.txtwidth || '60px'), lineHeight : '20px', textAlign : 'right', 'float' : 'left'}).appendTo(contain);
				}
...
var nicEditorSelect = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		this.contain = new bkElement('div').setStyle({width: '200px', height : '20px', cursor : 'pointer', overflow: 'hidden'}).addClass('selectContain').addEvent('click',this.toggle.closure(this)).appendTo(this.margin);
	...
	open : function() {
		this.pane = new nicEditorPane(this.items,this.ne,{width : '200px', padding: '0px', borderTop : 0, borderLeft : '1px solid #ccc', borderRight : '1px solid #ccc', borderBottom : '0px', backgroundColor : '#fff'});
		
		for(var i=0;i<this.selOptions.length;i++) {
			var opt = this.selOptions[i];
			var itmContain = new bkElement('div').setStyle({overflow : 'hidden', borderBottom : '1px solid #ccc', width: '200px', textAlign : 'left', overflow : 'hidden', cursor : 'pointer'});
...
var nicEditorFontFormatSelect = nicEditorSelect.extend({
	sel : {'p' : 'Paragraph', 'pre' : 'Pre', 'h1' : 'Heading&nbsp;1', 'h2' : 'Heading&nbsp;2', 'h3' : 'Heading&nbsp;3', 'h4' : 'Heading&nbsp;4', 'h5' : 'Heading&nbsp;5', 'h6' : 'Heading&nbsp;6'},
	
	init : function() {
		this.ne.addEvent('selected', this.selected.closure(this))
			.addEvent('keyup', this.keyDown.closure(this));
		
		this.setDisplay('Paragraph');
		for(itm in this.sel) {
	...
	selected : function(ins,t) {
		...
		// console.log("selected..." + t.tagName);
		
		this.checkNodes($BK(t));
	},
	
	keyDown : function(e,t) {
		var node = this.ne.selectedInstance.selElm();
		if(!node.tagName){
	...
	checkNodes : function(e) {
		...
		if('tagName' in e){
			// console.log("check tag name..." + e.tagName);
			...
			if(name == 'p'){
				...
				this.setDisplay('Heading&nbsp;1');
				return;
			}
			else if(name == 'h2'){
				this.setDisplay('Heading&nbsp;2');
				return;
			}
			else if(name == 'h3'){
				this.setDisplay('Heading&nbsp;3');
				return;
			}
			else if(name == 'h4'){
				this.setDisplay('Heading&nbsp;4');
				return;
			}
			else if(name == 'h5'){
				this.setDisplay('Heading&nbsp;5');
				return;
			}
			else if(name == 'h6'){
				this.setDisplay('Heading&nbsp;6');
				return;
			}
		}else{
			// console.log("no tag name..." + e.tagName);
		}
		
		var sel = e.parentTag('P');
		if(sel){
			this.setDisplay('Paragraph');
			return;
		}
...
var nicLinkButton = nicEditorAdvancedButton.extend({
	...
	addPane : function() {
		...
		var opt = this.getAnchors();
		
		this.addForm({
			'' : {type : 'title', txt : 'Add/Edit Link'},
			'href' : {type : 'text', txt : 'URL', value : 'http://', style : {width: '400px'}},
			'anchor' : {type : 'select', txt : 'Anchor', options : opt, style : {width : '300px'}},
			'title' : {type : 'text', txt : 'Title'},
			'target' : {type : 'select', txt : 'Open In', options : {'' : 'Current Window', '_blank' : 'New Window'},style : {width : '100px'}}
		},this.ln);
		
		var anchorSel = this.inputs['anchor'];
		
		anchorSel.addEvent('click', this.anchorListner.closureListener(this) );
		
	},
	
	anchorListner : function ()
	{
		var anchorSel = this.inputs['anchor'];
		var val = "#" + anchorSel.value;
		
		this.inputs['href'].value = val;
	},
	
	getAnchors : function()
	{
		var options = {};
		var list = this.ne.selectedInstance.getElm().getElementsByTagName("a");
		for(var i=0;i<list.length;i++) {
			var objtag = $BK(list[i]);
			if(objtag.hasAttribute("name")){
				var anchorName = objtag.getAttribute("name");
				if(anchorName){
					options[anchorName] = anchorName;
				}
			}
		}
...
var nicImageButton = nicEditorAdvancedButton.extend({
	...
	addPane : function() {
		...
		this.addForm({
			...
			'src' : {type : 'text', txt : 'URL', 'value' : 'http://', style : {width: '500px'}},
			'alt' : {type : 'text', txt : 'Alt Text', style : {width: '500px'}},
			'title' : {type : 'text', txt : 'Title Text', style : {width: '500px'}},
			'border' : {type : 'text', txt : 'Border', style : {width: '50px'}},
			'align' : {type : 'select', txt : 'Align', options : {none : 'Default','left' : 'Left', 'right' : 'Right'}},
			'width' : {type : 'text', txt : 'Width', style : {width: '100px'}},
			'height' : {type : 'text', txt : 'Height', style : {width: '100px'}}
		},this.im);
	},
	...
	submit : function(e) {
		...
		if(this.im) {
			...
			if(this.inputs['height'].value != ''){
				...
				this.im.removeAttribute('height');
			}
