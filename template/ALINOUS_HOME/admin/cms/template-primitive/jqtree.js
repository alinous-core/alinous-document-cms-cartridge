...
function initJqTree()
{
	...
	$.getJSON(
		'/admin/cms/template-primitive/makejson.alns',
		function(data) {
			$tree = $('#pageTree').tree({
				dragAndDrop: true,
				data: data,
			...
			$tree.on(
				...
				function(e) {
					...
					if (node) {
						var url = "/admin/cms/template-primitive/dialog/editNode.html";
						$.ajax({
							type: 'POST',
							url: url,
							data: {
			...
			$tree.bind(
				'tree.move',
				function(event) {
					var url = "/admin/cms/template-primitive/moveHandler.alns";
    ...
    $.contextMenu({
        ...
        items: {
        	"import": {name: "Import template from zip file", icon: "/admin/cms/article/img/world.png"},
